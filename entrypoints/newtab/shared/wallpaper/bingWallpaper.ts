...
class BingWallpaperURLGetter {
  ...
  private readonly useWebProxy =
    (import.meta.env as Record<string, string | boolean | undefined>).WEB === true
  ...
  private bingUrl(path: string) {
    const normalizedPath = path.startsWith('/') ? path : `/${path}`
    if (this.useWebProxy) return `/api/bing${normalizedPath}`
    return `https://www.bing.com${normalizedPath}`
  }

  private async fetchImage(url: string, timeout = 3000) {
  ...
  public async refresh(force = false) {
    ...
    try {
      ...
      // 跨标签页冷却期检查（force=true 时绕过）
      if (!force) {
        const cached = await bingInfoCache.getValue()
        if (Date.now() - cached.lastCheckTime < this.COOLDOWN_MS) return
      }

      // 立即更新 lastCheckTime，防止其他标签页在本次请求完成前重复发起
      const checkTime = Date.now()
      {
        const cached = await bingInfoCache.getValue()
        await bingInfoCache.setValue({ ...cached, lastCheckTime: checkTime })
      }

      try {
        data = await enhancedFetch<BingWallpaperResp>(
          this.bingUrl('/HPImageArchive.aspx?format=js&idx=0&n=1'),
        ...
        console.error(error)
        // API 请求失败：重置 lastCheckTime，允许下次打开新标签页时重试
        try {
          const cached = await bingInfoCache.getValue()
          await bingInfoCache.setValue({ ...cached, lastCheckTime: 0 })
        } catch {}
        const cache = await bingInfoCache.getValue()
      ...
      const imgUrl = this.toWebp(this.bingUrl(data.images[0]!.url))

      try {
        const response = await this.fetchImage(imgUrl)
        if (!response.ok) {
          // 兜底：网络请求失败，直接使用在线URL
